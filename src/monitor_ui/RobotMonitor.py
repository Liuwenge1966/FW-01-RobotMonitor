...
class RobotMonitor(QMainWindow, Ui_MainWindow):
    ...
    def setup_connections(self):
        ...
        self.btn_TurnLeft.clicked.connect(lambda: self.set_movement(speed=0.0, steer=15))
        self.btn_TurnRight.clicked.connect(lambda: self.set_movement(speed=0.0, steer=-15))
        self.btn_Stop.clicked.connect(lambda: self.set_movement(speed=0.0, steer=0.0))
        self.btn_HeadLeft.clicked.connect(lambda: self.set_movement(speed=0.1, steer=15))
        self.btn_HeadRight.clicked.connect(lambda: self.set_movement(speed=0.1, steer=-15))
        self.btn_BackLeft.clicked.connect(lambda: self.set_movement(speed=-0.1, steer=-15))
        self.btn_BackRight.clicked.connect(lambda: self.set_movement(speed=-0.1, steer=15))
    ...
    def on_status_message(self, topic, data):
        ...
        try:
            ...
            # 在接收消息列表中显示原始数据（只保留最近10条）
            message_text = f"[{timestamp}] {json.dumps(data, ensure_ascii=False)}"
            self.add_message_to_list(self.lvRevMsg, message_text)
            
            # 更新各个状态显示标签
            self.lb_Gear.setText(f"当前档位：{GEAR_MAP.get(data.get('Gear', 0))}")
    ...
    def add_message_to_list(self, list_widget, message_text):
        """向列表中添加消息，只保留最近的10条，最新消息在最前面"""
        # 在开头插入新消息
        list_widget.insertItem(0, QListWidgetItem(message_text))
        
        # 如果超过10条消息，删除最后一条
        if list_widget.count() > 10:
            # 删除最后一条（索引为count()-1）
            item = list_widget.takeItem(list_widget.count() - 1)
            del item  # 显式删除项目以释放内存
    ...
    def send_control_command(self):
        ...
        try:
            ...
            if success:
                ...
                # 在发送消息列表中显示已发送的命令（只保留最近10条）
                message_text = f"[{timestamp}] {json.dumps(self.current_control_cmd, ensure_ascii=False)}"
                self.add_message_to_list(self.lvSendMsg, message_text)
